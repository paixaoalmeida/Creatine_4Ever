...
def send_message(*, numero: str):
    ...
    pywhatkit.sendwhatmsg(f'{numero}', f'Creatina disponivel!\n {link_creatine}', int(now_hour), int(now_min) + 3)
...
def do_stuff(text_list: list) -> None:
    ...
    QUANDO CHEGAR, then it's available
    """
    while True:
        if 'AVISE-ME QUANDO CHEGAR' in text_list:
            print("NÃO DISPONÍVEL! " + datetime.strptime(str(datetime.now()), '%Y-%m-%d %H:%M:%S.%f').strftime(
                '%d/%m %H:%M'))
            time.sleep(120)
        elif 'AVISE-ME QUANDO CHEGAR' not in text_list:
            send_message(numero='+5511984218991')
            exit()
